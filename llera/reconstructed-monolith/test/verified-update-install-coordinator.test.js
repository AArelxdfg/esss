...
(async()=>{
 ...
 let receiptBlocked=false; const malformed=new VerifiedUpdateInstallCoordinator({updater:{...updater,verifySignedManifest(){return{verified:true}}},installer,watchdog}); try{await malformed.apply({manifest,signatureBase64:'signed-manifest'})}catch(e){receiptBlocked=/verified manifest receipt/.test(e.message)} assert.strictEqual(receiptBlocked,true);
