...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    window->set_title("AArel OS — ForgeShell");
    ...
    root->layout()->set_margins({ 32, 32, 32, 32 });

    auto& title = root->add<GUI::Label>("AArel OS — Forge");
    title.set_font(title.font().bold_variant());
    title.set_text_alignment(Gfx::TextAlignment::CenterLeft);
    title.set_fixed_height(36);

    auto& subtitle = root->add<GUI::Label>("SerenityOS foundation • developer preview");
    ...
    auto& llera = root->add<GUI::Button>("LLera — runtime gate not passed");
    llera.set_enabled(false);
    llera.set_fixed_height(44);
