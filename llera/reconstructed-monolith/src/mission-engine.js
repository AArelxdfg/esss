...
class MissionEngine {
  ...
  async createMission({ title, goal, mode = 'work', steps = [], budget = {} } = {}) {
    ...
    const normalizedSteps = steps.map((step, index) => {
      ...
      return {
        id: step.id || stableId('step', `${id}:${index}:${name}`), name, status: 'pending', dependencies: [...dependencies],
        attempts: 0, startedAt: null, completedAt: null, lastError: null, checkpointId: null
      };
    });
    ...
    const mission = {
      id, title, goal, mode, status: 'pending', createdAt, updatedAt: createdAt, startedAt: null, completedAt: null,
      currentStepId: null, resumeCount: 0,
      budget: {
      ...
      steps: normalizedSteps, checkpoints: [], toolTrace: []
    };
  ...
  async beginStep(missionId, stepId) {
    ...
    mission.currentStepId = step.id; mission.updatedAt = this.now();
    await this._persist();
    return clone(step);
  }
  ...
  async appendToolTrace(missionId, entry = {}) {
    ...
    const record = {
      ...
      at: this.now(), stepId: entry.stepId || mission.currentStepId || null, tool: entry.tool,
      argumentsHash: entry.argumentsHash || null, outcome: entry.outcome || 'observed', material: Boolean(entry.material),
      verification: Boolean(entry.verification), evidenceIds: [...(entry.evidenceIds || [])]
    };
    mission.toolTrace.push(record); mission.updatedAt = record.at;
    await this._persist();
    return clone(record);
  }
  ...
  async completeStep(missionId, stepId, result = {}) {
    ...
    step.status = 'completed'; step.completedAt = completedAt; step.lastError = null; mission.currentStepId = null;
    if (mission.steps.every(s => s.status === 'completed')) { mission.status = 'completed'; mission.completedAt = completedAt; }
    const checkpoint = this._buildCheckpoint(mission, { type: 'step-complete', stepId, result });
    step.checkpointId = checkpoint.id; mission.checkpoints.push(checkpoint); mission.updatedAt = checkpoint.at;
    await this._persist();
    return clone(mission);
  }
  ...
  async failStep(missionId, stepId, error) {
    ...
    step.lastError = String(error?.message || error || 'unknown error'); mission.currentStepId = null;
    mission.status = step.status === 'failed' ? 'failed' : 'running'; mission.updatedAt = this.now();
    await this._persist();
    return clone(mission);
  }
  ...
  async pauseMission(id, reason = 'user-pause') {
    ...
    if (mission.currentStepId) {
      ...
      step.status = 'pending'; step.lastError = `interrupted:${reason}`; mission.currentStepId = null;
    }
    mission.status = 'paused'; mission.updatedAt = this.now();
    await this._persist();
    return clone(mission);
  }
  ...
  _repairInterruptedState() {
    for (const id of this.state.order) {
      ...
      if (mission.status === 'running' || mission.currentStepId) {
        if (mission.currentStepId) {
          ...
          if (step && step.status === 'running') {
            ...
            if (durableCompletion) {
              step.status = 'completed'; step.completedAt = step.completedAt || durableCompletion.at; step.lastError = null; step.checkpointId = durableCompletion.id;
            } else {
              step.status = 'pending'; step.lastError = 'interrupted:process-restart';
            }
          }
        }
        ...
        } else mission.status = 'interrupted';
        mission.updatedAt = this.now();
      }
    }
  }
  ...
  _buildCheckpoint(mission, payload = {}) {
    const at = this.now();
    return {
      id: stableId('checkpoint', `${mission.id}:${mission.checkpoints.length}:${at}`), at, status: mission.status,
      currentStepId: mission.currentStepId, completedStepIds: mission.steps.filter(s => s.status === 'completed').map(s => s.id), payload: clone(payload)
    };
  }
  ...
  _assertMutationAvailable() {
    if (!this.persistenceInFlight) return;
    const error = new Error('mission persistence transaction already in progress');
    error.code = 'MISSION_PERSISTENCE_IN_PROGRESS';
    throw error;
  }
