...
const DEFAULT_SETTINGS = Object.freeze({ theme: 'system', activityDensity: 'balanced', sidebarCollapsed: false, showSummary: false, showContext: false, mode: 'chat', textScale: 1, motion: true, defaultModel: null });
...
class MonolithService {
  ...
  async updateSettings(input) { const allowed = ['theme', 'activityDensity', 'sidebarCollapsed', 'showSummary', 'showContext', 'mode', 'textScale', 'motion', 'defaultModel']; for (const key of allowed) if (Object.hasOwn(input || {}, key)) this.state.settings[key] = input[key]; if (!['dark', 'light', 'system'].includes(this.state.settings.theme)) this.state.settings.theme = 'system'; if (!['compact', 'balanced', 'detailed'].includes(this.state.settings.activityDensity)) this.state.settings.activityDensity = 'balanced'; if (!['chat', 'work'].includes(this.state.settings.mode)) this.state.settings.mode = 'chat'; this.state.settings.showSummary = Boolean(this.state.settings.showSummary); this.state.settings.showContext = Boolean(this.state.settings.showContext); this.state.settings.textScale = Math.max(.9, Math.min(1.2, Number(this.state.settings.textScale) || 1)); this._save(); this._emit('settings.changed', this.state.settings); return this.snapshot(); }
  ...
  async send({ content, attachmentIds = [], model = null }) {
    ...
    try {
      ...
      if (!this.runtime.completeInference(inferenceId, inferenceTask.generation)) { conversation.messages = conversation.messages.filter(message => message.id !== assistant.id); return this._block(conversation, 'STALE_INFERENCE_GENERATION', 'The response was discarded because the local runtime generation changed before completion.'); }
      ...
      if (inferenceTask) this.runtime.completeInference(inferenceId, inferenceTask.generation);
