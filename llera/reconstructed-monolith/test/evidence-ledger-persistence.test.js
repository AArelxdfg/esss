...
try {
  ...
  const entry = first.add({stepId:'s1',kind:'artifact',target:'C:/LLera/app.exe',bytes:Buffer.from('artifact-v1')});
  ...
  assert.strictEqual(restarted.verifyBinding(entry.id,{target:'C:/LLera/app.exe',bytes:Buffer.from('artifact-v1')}).ok,true);
  ...
  source.add({stepId:'s',kind:'state',target:'x',bytes:Buffer.from('x')});
  assert.throws(
