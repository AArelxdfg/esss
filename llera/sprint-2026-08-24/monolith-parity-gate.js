...
function scan(root, contract) {
  ...
  for (const [name, tools] of Object.entries(contract.requiredToolFamilies)) {
    ...
    for (const tool of tools) {
      ...
      if (re.test(text)) { foundTools.add(tool); family[name].found.push(tool); }
    }
  }
  const markers = {};
  for (const marker of contract.requiredBehaviorMarkers) markers[marker] = new RegExp(marker,'i').test(text);
