...
class HostguardRuntimeCoordinator {
  constructor({ governor, runtime, vision = null, downloader = null, setRuntimePriority = null } = {}) {
    ...
    this.lastApplied = { pressure:null, downloadWorkers:null, runtimePriority:null, visionUnloadedForCritical:false };
    this.history = [];
  }
  async sample(metrics = {}) {
    ...
    if (policy.pressure !== this.lastApplied.pressure) {
      ...
      actions.push({ type:'runtime-pressure', pressure:policy.pressure, aborted:Array.isArray(pressureResult && pressureResult.aborted) ? [...pressureResult.aborted] : [] });
  ...
  status() { return { policy:{...this.governor.policy()}, canStartVision:this.canStartVision(), lastApplied:{...this.lastApplied}, samples:this.history.length }; }
}
